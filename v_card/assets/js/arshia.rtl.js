    ...
    $("#main").animate(
      {
        ...
        right: (windowWidth * 5) / 100 + 315 + "px",
      },
      700
    );
  } else {
...
function sidebarMenu() {
  ...
  $(".prev-page").on("click", function () {
    console.log("Previous Page", $first);
    $lastHash = 1;

    var currentHref = $(".menu > li a.active").attr("href");
    console.log("Current active href: " + currentHref);
    ...
    if ($(".menu > li a.active").attr("href") !== $first) {
      ...
      console.log("Already at first, navigating to last");
      // $(".menu > li a").removeClass("active");
      // $(".menu > li:last-child a").addClass("active");
      // $("#main > section.active").addClass(func[1]).removeClass("active");
      // $("#main > section:last-child").addClass("active " + func[0]);
      // changeWindowLocation($last); // navigate to the last section
      $(".menu > li a.active").removeClass("active");
      $("main > section.active").addClass(func[1]).removeClass("active");
      $("main > section:last-child").addClass("active " + func[0]);
      $(".menu > li:last-child a").addClass("active");
  ...
  $pfilter.find("a").on("click", function () {
    ...
    if ($(window).width() > 768) {
      if (filterValue === "*") {
        ...
        var $allElements = $grid.find(".filtered");
        var $lastElement = $grid.find(".last");
        ...
        var $elementsWithClassX = $grid.find(`${filterValue}`);
        console.log($elementsWithClassX.length);

        // Do something with those elements
        $elementsWithClassX.each(function () {
          // Example: Logging the HTML of each element
          if ($(this).hasClass("fruit")) {
            // This element has the 'specific-class'
            console.log("Element has the specific class");
            $(this).addClass("last");
          }
...
function modifyLinksForMobile() {
  ...
  if ($(window).width() <= 768) {
    ...
    $(".portfolio-item a").each(function () {
      $(this)
        .removeClass("mfp-iframe")
        .removeClass("portfolio-magnific")
        .addClass("ajax-page-load");
    });
  }
}
...
function addCloseBtn() {
  ...
  if ($(window).width() <= 768) {
    // Append the close button HTML to each ajax-page-nav
    $(".ajax-page-nav").each(function () {
      var grandfather = $(this).parents().eq(1);
      console.log("ajax-page-nav", grandfather);
      var grandfatherID = grandfather.attr("id");
      var isBlog = grandfatherID === "blog-single";
      var href = isBlog ? "../index.html#blog" : "index.html#portfolio";
      console.log("href", href);
      // Define the HTML for the close button
      var closeButtonHtml =
        '<div class="nav-item ajax-page-close-button">' +
        `<a class="ajax-page-close-button" href=${href}>` +
        '<i class="bi bi-x"></i>' +
        "</a>" +
        "</div>";

      if ($(this).find(".ajax-page-close-button").length === 0) {
        $(this).append(closeButtonHtml);
      }
    });

    // Modify links in the dynamically loaded content
    $(".portfolio-item a").removeClass("mfp-iframe").addClass("ajax-page-load");
  }
}

function hideMenuBtn() {
  console.log("hideMenuBtn", window.location.href);
  if ($(window).width() <= 768 && window.location.href.includes("/v_card")) {
    $(".menu-toggle").addClass("menu-hide");

    $(".color-switcher .pallet-button").addClass("right-zero");
  } else {
    $(".menu-toggle").removeClass("menu-hide");

    $(".color-switcher .pallet-button").removeClass("right-zero");
  }
}
...
//     $.ajax({
//       type: "POST",
//       url: "http://localhost:3000/sendemail", // Update with your server URL
//       data: formData,
